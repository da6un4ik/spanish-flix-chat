...
export const IdiomPractice = ({ 
  ...
  return (
    <div className="flex flex-col h-full bg-background text-foreground p-6 pt-12">
          ...
          {idiom.exercises.map((_, idx) => (
            <div 
              ...
              className={`h-1.5 flex-1 rounded-full transition-all duration-500 ${
                idx <= currentStep ? 'bg-primary shadow-[0_0_8px_hsl(var(--primary)/0.5)]' : 'bg-secondary'
              }`} 
            />
          ))}
        </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <p className="text-primary uppercase tracking-[0.3em] text-[10px] font-black mb-4">Упражнение {currentStep + 1}</p>
              ...
              <button
                ...
                className={`w-full p-6 rounded-2xl text-left text-lg font-bold border-2 transition-all active:scale-[0.98] ${
                  selectedOption === option
                    ...
                    : 'border-border bg-secondary hover:bg-secondary/80 hover:border-muted-foreground'
                }`}
              >
                <div className="flex items-center justify-between">
                  {option}
                  {selectedOption === option && isCorrect && <CheckCircle2 className="text-emerald-500 w-6 h-6" />}
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="text-center pb-8 text-muted-foreground text-[10px] uppercase font-black tracking-[0.4em]">
